...
const OtpScreen = ({navigation, route}) => {
  const {name, mobile, password, email} = route.params;
  ...
  const verifyregistraion = () => {
    ...
    let body =
      'verification_code=' + data.otp + '&email=' + email + '&mobile=' + mobile;
    axios
      .post('/website/Services/validate_otp', body)
      .then(response => {
        setData({
          ...data,
          loading: false,
        ...
        if (response.data.status == 1) {
          let body1 =
            ...
            mobile;
          axios
            .post('/website/Services/sign_up', body1)
            .then(response => {
              setData({
                ...data,
                loading: false,
  ...
  const resendOtp = () => {
    ...
    axios
      .post('/website/Services/save_otp', body)
      .then(response => {
        setData({
          ...data,
          loading: false,
